...
## 🔗 Pipedrive Integration - Redirect URI Konfiguration
...
2. Öffne deine App unter **My Apps**
...
Falls du einen temporären Cloudflare Tunnel verwendest:
...
Du erhältst eine URL wie `https://random-words.trycloudflare.com`. Dann:

1. `.env.local` mit der neuen URL aktualisieren
2. Pipedrive Developer Hub mit der neuen URL aktualisieren
3. App neu starten

> ⚠️ **Wichtig**: Die Redirect URI muss in `.env.local` und Pipedrive Developer Hub **exakt übereinstimmen**!

### Wo wird die Redirect URI angezeigt?

- **Admin-UI**: Unter `/admin` → Pipedrive Integration → Konfiguration
